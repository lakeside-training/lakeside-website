...
const Courses = () => {
    ...
    useEffect(() => {
        const API = async () => {
            ...
            const enable = data.data.filter((i) => i.course_enable === "true")
            const filter = enable.filter((i) => i.course_type === "Course")
    ...
    const handleSelectCourse = (id) => {
        ...
        const getCourseData = async () => {
            const { data } = await axios.post("/getParticular/data", { id })
                ...
                <>
                    <div className="mx-auto text-center mt-20">
                        <p className=" text-xl font-semibold text-center text-gray-900">
                            You have not purchased any course yet!☹️
                        </p>
                        <Link to="/courses">
                          ...
                          <p className="text-[#4F46E5] text-sm mt-0.5">22%</p>
                                            ...
                                            <div className="buttons w-full mt-[10px]">
                                                <Button
                                                    ...
                                                    variant="gradient"
                                                >
                                                    {firstLabs.find((i) => i.course_id === data?.id)?.start +
                                                        " Course" ?? null}
                                                </Button>
                                                <Link to={`/course-details?id=${data.id}&hide=hide&course=mycourse`}>
                                                    <Button
                                                ...
                                                <div className="text-center">
                                                    <p>Duration {data.duration} week</p>
                                                </div>
                                            </div>
                                        </div>
                                    )
              ...
              "
                    ...
                    <p className="text-[#4F46E5] text-xs">22%</p>
                                ...
                                <p
                                    ...
                                    className="text-[#4F46E5] text-md cursor-pointer"
                                >
                                    View Certificate
                                </p>
                            </div>
                            <div className="overflow-x-auto pb-5 px-2">
                                    ...
                                    {course?.modules?.map((data, index) => {
                                        return (
                                            <Accordion.Panel key={index}>
                                                <Accordion.Title>
                                                    <div className="flex-col sm:flex !justify-start relative sm:justify-between items-center">
                              ...
                              <div className=" flex justify-start sm:absolute left-[45vw] sm:left-[50vw] md:left-[59vw] top-3">
                                <a
                                  href={data.lab_link}
                                  target="_blank"
                                  className="w-[15%] ml-9 mt-2 sm:mt-0 text-[#4F46E5] text-md font-semibold flex  md:flex items-center cursor-pointer"
                                  rel="noreferrer"
                                >
